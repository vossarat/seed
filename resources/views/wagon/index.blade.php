                            ...
                            <div class="row wagon-table-row {{ $wagon->active ? '' : 'closed'}}" id="row-wagon-{{ $wagon->id }}">
                                ...
                                <div class="row">
                                 ...
                                 <div class="row">
	                                <div class="col-xs-12">
	                                	@if( $wagon->active )
	                                	<a class="toogle-order-detailed" href="/api/views/{{ $wagon->id }}/wagons" order-id={{ $wagon->id }}>{{ 'Перевезу:'.$wagon->corn['name'] }}, {{ $wagon->count . ' тонн' }}</a>
	                                	@else
	                                ...
	                                <div class="order-detailed toogle-off">
                                		<ul>
                            				...
                            				@if( $wagon->date_delivery )
                            				<li><u>Дата поставки:</u>
                            					{{ date('d.m.Y',strtotime($wagon->date_delivery)) }}
                            				</li>
                            				@endif                                			
                                			
                                			<li><u>Тип транспорта:</u>                                				
                                				{{ $wagon->transport_tip === 1 ? 'авто' : 'ЖД' }} 
                                			</li>
                                			<li><u>Место формирования груза:</u>                                				
                                				@if($wagon->elevator_id)
                                					<li>Элеватор: {{ $wagon->elevator->title }} </li>
                                				@endif  
                                					<li>Пункт: {{ $wagon->point->name }} </li>
                                				@if($wagon->address_supply)
                                					<li>Адрес подачи: {{ $wagon->address_supply }} </li>
                                				@endif  
                                				@if($wagon->address_delivery)
                                					<li>Адрес доставки: {{ $wagon->address_delivery }} </li>
                                				@endif                                				
