...
<div class="form-group">
	<form action="{{ route('dashboard_farmer.create') }}">
...
<table id="viewtable" class="table table-striped">
	...
	<tbody>
		@foreach($viewdata as $farmer)
		<tr>
			<td>{{ $farmer->user->name }}</td>
			<td>{{ $farmer->user->email }}</td>            
		
			<td>
				<form action="{{ route('dashboard_farmer.edit', $farmer->id) }}">
                	<button type="submit" class="btn btn-warning"><i class="fa fa-edit"></i></button>
                </form>
			</td> 
			
			<td>
				<form action="{{ route('dashboard_farmer.destroy', $farmer->id) }}" method="POST">
